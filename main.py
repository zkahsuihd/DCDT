...
def main(data_loader_dict, args, optim_config, cuda, writer, one_subject, seed=3):
    ...
    if args.dataset_name == 'seed3':
        ...
        iteration = 3
    acc = trainDCDT(data_loader_dict, optim_config, cuda, args, iteration, writer, one_subject)
    return acc

if __name__ == '__main__':
    cuda = torch.cuda.is_available()
    parser = argparse.ArgumentParser(description='DCDT')

    #config of experiment
    parser.add_argument("--way", type=str, default='DCDT/seed3', help="name of current way")
    parser.add_argument("--index", type=str, default='30', help="tensorboard index")
    ...
    parser.add_argument("--epoch_fineTuning", type=int, default=300, help="epoch of the fine-tuning phase")
    ...
    parser.add_argument("--alpha_decay_epochs", type=int, default=25)  #25
    ...
    parser.add_argument('--lambda_disp_pretrain', type=float, default=0.5,
                        help='Dispersive loss weight for pretraining')
    parser.add_argument('--lambda_disp_finetune', type=float, default=0.1,
                        help='Dispersive loss weight for finetuning')

    parser.add_argument('--temperature_pretrain', type=float, default=0.3,
                        help='Temperature parameter for pretraining')
    parser.add_argument('--temperature_finetune', type=float, default=0.1,
                        help='Temperature parameter for pretraining')
    ...
    args.seed3_path = "./datasets/SEED/ExtractedFeatures/"
    args.seed4_path = "./datasets/SEED_IV/eeg_feature_smooth/"
    if cuda:
    ...
    if args.dataset_name == "seed3":
        ...
        args.epoch_preTraining = 50  #epoch of the pre-training phase  50
    elif args.dataset_name == "seed4":
        args.path = args.seed4_path
        args.cls_classes = 4
        args.time_steps = 10
        args.batch_size = 256  #batch_size
        args.epoch_preTraining = 50  #epoch of the pre-training phase
    else:
